...
public class SLL<T extends Comparable<T>> implements Iterable<T>
{
    ...
    /**
     * Add a new object t, to the list. It will be added in the order specified
     * by c.
     * 
     * @param t
     *            the object to add
    ...
    /*
     * This method does the heavy lifting for adding nodes in order. Adds n in
     * the correct order based on c, or the natural order if c is null.
     */
	...
	{
	    n = head;
	    if(head == tail) tail = head.getNext();
	    head = head.getNext();
	    size--;
	}
	return n;
    ...
    {
	int r = 0;
	if(c == null) r = n1.getData().compareTo(n2.getData());
	else r = c.compare(n1.getData(), n2.getData());
	// System.err.println( "Comparator" + c.getClass().getName());
	// System.err.println( "Comparing " + n1.getData() + " to " +
	// n2.getData() + " = " + r);
	return r;
    }
}
